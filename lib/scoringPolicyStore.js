...
export async function saveScoringPolicy({ region, policy, userId, userEmail, audit = true, source = 'policy_editor' }) {
  const normalizedRegion = normalizeScoringRegion(region);
  ...
  if (audit) {
    const { error: auditError } = await supabaseAdmin.from('audit_log').insert({
      ...
      new_value: JSON.stringify({ version_label: data.version_label, updated_at: data.updated_at, source }),
