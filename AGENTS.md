...
groups → venues (1:1) → cases
                          ...
                          ├── document_requests (one-to-many renewal requests)
...
**`documents`** — tracks both uploaded originals and generated outputs. `document_category`: `'original'` (uploaded by analyst) or `'output'` (generated by platform). `storage_path` follows the convention: `originals/{case_id}/{filename}` or `outputs/{case_id}/{filename}` in Supabase Storage. `expiry_date` is NULL for non-expiring documents. `renewal_status` (`pending` / `received`) is set by the document workflow. `document_type` is free-text validated at the app layer; known values documented in the migration file.

**`document_requests`** — renewal/request workflow rows for expiring or missing required documents. Tracks `status` (`pending`, `provided`, `closed`, `not_needed`), request email draft fields, requester, due date, and linked `provided_document_id` after proof upload. Provided requests stay visible in the active UI for 7 days; the row remains in the database for audit/history.
...
| **Document expiry** | The date (`documents.expiry_date`) a regulatory document becomes invalid. Platform surfaces required docs expiring within the 90-day cap window and flags the final 7 days as a renewal warning. |
...
- **Region badge colours:** Defined as `--mz-region-{uae|usa}-{bg|border|text}` tokens in `app/globals.css`. UAE uses sandy ochre (`#b89a60`); USA uses muted slate (`#6e8ca6`). Both are intentionally quieter than status tokens (0.10 bg / 0.28 border opacity). Use `RegionBadge` from `app/_components/RegionBadge.jsx` everywhere — never hardcode region colours in page components.
